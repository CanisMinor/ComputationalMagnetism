...
#print(endl
#print('****************************************************************************' )
#print('* Constrained Monte Carlo Simulation for an FePt System with KKR Jij Input *' )
#print('****************************************************************************' )
#print(endl
#print(endl

# if(RotMatrix(theta,phi,irot,rot,mz_unit)!=0)
#    print('error in evaluation of rotation matrix' )
#    print('terminating program' )
#    return 1
# }

print('mz_unit: ' + mz_unit[0] + '\t' + mz_unit[1] + '\t' + mz_unit[2] )

print('counting no of neighbours\n' )
nFeFe = ex.CountExch('../data/effectiveJij.dat')
print('reading in ' + nFeFe + ' Fe-neighbours for each of ' + pr.N_s + ' Fe-sites' +'\n')
...
print('reading in ' + nFeFe + ' effective exchange tensors from effectiveJij.dat' )
JijFeFe_eff, inform_FeFe, interat_FeFe = ex.ReadEffExch('effectiveJij.dat', nFeFe)


#convert from Rydbergs to Joules/pr.mu
for i in range(0, nFeFe):
    for j in range(0,9):
        JijFeFe_eff[i,j] = JijFeFe_eff[i,j] * Ry
...
with open('magn-vs-T.dat') as Tfile:
for iT in range(0,1002,10):
    temp = float(iT)
    beta = 1.0 / (pr.kB*temp)
        ...
        for j in range(0, pr.N_s):
            ...
            for k in range(0, nFeFe):
                ...
                if JijFeFe_eff[Jij_line][8]==0:
                    #print('error: neighb_site, nsp, j, Jij_line are ' + neighb_site + ', ' + nsp + ', ' + j + ', ' + Jij_line + ', ' + nsp*nFeFe+k )
